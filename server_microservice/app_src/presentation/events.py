...
@event_router.get("/full/{event_id}", response_model=FullEventData)
async def show_full_event(
    event_id: UUID, event_repo: EventService = Depends(EventService)
) -> FullEventData:
    return await event_repo.get_full_event(cmd=GetEventById(id=event_id))
